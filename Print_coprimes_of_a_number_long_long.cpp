// Print_coprimes_of_a_number_long_long.cpp
...
// it reaches 1. If the Orbit On(m) equals the increment but not 1, then that
// number is a coprime of n. It accumulates the sum of all these Orbits On(m)=t 
// into total. If n is prime, this sum should equal n(n-1)/2 + 1. If n is a 
// composite number then the sum of all the Orbits will be less than n(n-1)/2 + 1.
...
int main() {
    ...
    for (m = 1; m <= n; m++) {
    	...
    	if ((t == m) && (t != 1)) {        // testing if the Orbit On(m) is equal 
		                                   //       to the increment but not 1.
             cout << "The number " << m << " is a coprime of " << n << endl;
        }
    ...
    value_if_prime = n*(n-1)/2 + 1; //formula for total of all Orbits if n is prime
