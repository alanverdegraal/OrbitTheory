// Count_Lemoine_Pairs_over_Range.cpp
// Created by Alan Verdegraal  June 3, 2023
//
// After entering a range of numbers, this program
// first skips any even number, then identifies and counts
// all Lemonine Pairs consisting of a prime and a biprime
...
// two primes, incrementing the count, and continuing
...
int main() {
    ...
    long long int orbit_value, count_of_Lemoine_Pairs=0;
	...
	for (n=starting_value; n<=ending_value; n++) {
		if (n%2==1) {
			cout << "For " << n << " the number of Lemoine Pairs is ";
			for (i=2; i<=n; i++) {
				orbit_value=get_orbit_value(i,n);
				if (orbit_value==1) {
					found_prime=check4prime(i);
				...
				if ((found_prime==true)&&(first_prime==i)) {
					...
					if (found_biprime==true) {
						count_of_Lemoine_Pairs++;
					}
				}
			}
			cout << count_of_Lemoine_Pairs << endl;
			count_of_Lemoine_Pairs=0;
		}
	}
}
