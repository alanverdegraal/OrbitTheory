// Range_Count_Amalgams_and_Primes_in_the_Orbits_for_n.cpp
...
// entered number n, where On(d)=d, going through
// a specified range of numbers for n.
// 
...
int main() {
	...
	int long long first_number, last_number, orbit_value;
	...
	cout << "Counting Primes and Amalgams for n = " << first_number;
	cout << " through " << last_number << endl;
	cout << "=============================================" << endl;
	
	for (k=first_number; k<=last_number; k++) {
		for (i=2; i<k; i++) {
			orbit_value=get_orbit_value(i,k);
			if (orbit_value==i) {
...
//					cout << i << endl;
		...
		cout << k << "\t" << total_primes << "\t" << total_amalgams << endl;
		total_primes=0;
		total_amalgams=0;
	}
}
