// Count_Half_Prime_Pairs_within_a_Range_2.cpp
...
// Only half of the Prime Pairs are displayed as it proceeds.
//
// This also saves the final output line that includes the 
// total primes to a dat file. The filename needs to be updated
// in the program if it is desired to be located and named elsewhere.
...
int main() {
	...
	long long int orbit_value;
	long long int total_pairs=0, found_1=0;
    ...
    ofstream outdata;
    outdata.open("C:\\Users\\averdegraal\\Desktop\\Mathematics\\C-Programs\\Output\\Number_Prime_Pairs.dat");
	if ( !outdata ) {
    ...
    for (j = beginning_value; j <= ending_value; j++) { 
        ...
        cout << "Total Prime Pairs for " << n << ":  " << (total_pairs * 2) + found_1 << endl;
		outdata << "Total Prime Pairs for " << n << ":  " << (total_pairs * 2) + found_1 << endl;
		total_pairs=0;
    }
    outdata.close();
}
