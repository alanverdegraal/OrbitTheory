// Count_Primes_btw_Perfect_Squares.cpp
// Created by Alan Verdegraal October 15, 2022
//
// Checking Legendre's Conjecture, this count the
// number of primes between two consecutive Perfect Squares.
...
int main() {
	bool found_prime = false;
	long long int beginning_value, ending_value, first_square, next_square, total_primes=0;
    long long int i, j, n;
    ...
    for (j = beginning_value; j <= ending_value; j++) {
		...
		cout << first_square << " " << next_square << " " << total_primes << endl;
		total_primes=0;
    }
}
