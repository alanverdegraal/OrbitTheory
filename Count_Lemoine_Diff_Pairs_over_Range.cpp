// Count_Lemoine_Diff_Pairs_over_Range_2.cpp
...
// all Lemoine Diff Pairs consisting of a prime when
// subtracted from a biprime gives the odd n.
// 
...
int main() {
    ...
    long long int orbit_value, count_of_Lemoine_Diff_Pairs=0;
	...
	for (n=starting_value; n<=ending_value; n++) {
		if (n%2==1) {
			cout << "For " << n << " the number of Lemoine Diff Pairs is ";
			for (i=2; i<=n; i++) {
				orbit_value=get_orbit_value(i,n);
				if (orbit_value==1) {
					found_prime=check4prime(i);
				...
				if ((found_prime==true)&&(first_prime==i)) {
					...
					if (found_biprime==true) {
						count_of_Lemoine_Diff_Pairs++;
					}
				}
			}
			cout << count_of_Lemoine_Diff_Pairs << endl;
			count_of_Lemoine_Diff_Pairs=0;
		}
	}
}
