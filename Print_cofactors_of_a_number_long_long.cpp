// Print_cofactors_of_a_number_long_long.cpp
// Created by Alan Verdegraal -- June 19, 2020
//
...
// it reaches 1. If the Orbit On(m) does not equal the increment i or
// or the Orbit On(m) = 1, then that number is a cofactor of n. 
// The cofactors divided by On(m) is a factor of n. The cofactors and their
// Orbits On(m) are displayed in the output.
//
// The program accumulates the sum of all these Orbits On(m)=t into total. 
// If n is // prime, this sum should equal n(n-1)/2 + 1. If n is a composite 
// number then the sum of all the Orbits will be less than n(n-1)/2 + 1.
...
int main() {
    ...
    for (m = 1; m <= n; m++) {
    	...
    	if ((t != m) && (t != 1)) {     // testing if the Orbit is equal 
		                        // d the increment, if not, then m is a factor
             cout << "The number " << m << " is a cofactor of " << n << "; On(";
			 cout << m << ") is " << t << endl;
        }
