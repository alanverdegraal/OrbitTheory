// Test_Zone_for_Key_Value_for_Reverse_Transformation.cpp
// Created by Alan Verdegraal August 9, 2023
//
// This program tests a Zone for a Key Value
// needed for a Reverse Transformation of the Square Sq(n).
//
...
// those values for the Key Value for a Reverse
// Transformation.
// 
// One difference between the Forward and Reverse
// Transformations is that in the Reverse process,
// the last column and last row of Sq(n) is ignored,
// thus, the indexing only goes to n-1. 
//
...
// key value for the reverse transformation is n - zone. 
...
int main() {
   ...
   cout << endl;
   cout << "Testing for Reverse Transformation of Sq(n) to Sq(n-1)" << endl;
   ...
   for (d = 1; d < n-1; d++) { 
		...
		cout << "d=" << d << "     ";
   		while (i < n-1) {
        	...
        	if ((k <= n-zone+1) && (zone==cur_zone)) {     // printing out the next sequence value
                if (k == key_value)	{
    ...
    if (key_value_found==true) {
        ...
        cout << "The number " << n-1 << " is a Composite number." << endl;
	}
	else {
