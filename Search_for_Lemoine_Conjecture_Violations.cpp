// Search_for_Lemoine_Conjecture_Violations.cpp
...
// any odd numbers that do not have any Lemoine Pairs.
// If Lemoine's Conjecture holds, none will be found
// in the specified range.
//
// All Lemoine Pairs consist of a prime and a biprime
// that add up to the odd n.
// 
...
//
// If so, the program subtracts the prime number from
...
// searching large ranges. If no Lemoine Pairs are
			...
			for (i=2; i<=n; i++) {
				...
				if ((found_prime==true)&&(first_prime==i)) {
					...
					if (found_biprime==true) {
						found_Lemoine_Pair=true;
						i = n+1;
					}
				}
			}
			if (found_Lemoine_Pair==false) {
				cout << "**** No Lemoine Pairs found for " << n << endl;
			}
			else {
				found_Lemoine_Pair==false;
			}
		}
		if (n%1000==0) {
