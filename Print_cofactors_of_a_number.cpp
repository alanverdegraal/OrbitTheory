// Print_cofactors_of_a_number.cpp
...
int main() {
    ...
    for (m = 1; m <= n; m++) {
    	...
    	if ((t != m) && (t != 1)) {          // testing if the Orbit is equal 
		                                     //   o the increment, if not, then m is a factor
             cout << "The number " << m << " is a cofactor of " << n << "; On(" << m << ") is " << t << endl;
        }
    ...
    int value_if_prime = n*(n-1)/2 + 1;      // formula for the total of all Orbits 
	                                         //       if n is prime
