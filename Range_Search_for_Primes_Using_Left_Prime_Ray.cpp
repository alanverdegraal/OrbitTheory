// Range_Search_for_Primes_Using_Left_Prime_Ray.cpp
// Created by Alan Verdegraal -- July 30, 2022
//
// This program uses the Left Prime Ray test to list prime numbers 
...
//
// For each n, the process starts from the lowest value of k = (n+1)/2 
// and runs up to k = n-1. It uses the arithmetic progressions modulo k 
...
int main() {
   ...
   for (j = beginning_value; j <= ending_value; j++) { 
      ...
      if (n == 2) {
         ...
         while ((cur_n <= n - 1) && (composite_found == false)) { 
            k = i + 1;
            while (k != 1) {  
               if (k > cur_n) {
