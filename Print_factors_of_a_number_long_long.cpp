// Print_factors_of_a_number_long_long.cpp
// Created by Alan Verdegraal -- June 20, 2020
...
// it reaches 1. If the Orbit On(m) equals 1, then that
// number is a factor of n. It accumulates the sum of all these Orbits On(m)=t
// in total. If n is prime, this sum should equal n(n-1)/2 + 1. If n is a 
// composite number then the sum of all the Orbits will be less than 
// n(n-1)/2 + 1. After checking, it will display a message whether the 
// number is prime or composite

// This uses 64 bit number size with long long int
// Maximum value of any register is 9223372036854775807
// Since the formula involves n * n-1 the maximum number able to be 
// processed is the square root of the limit or 3032000000.
...
int main() {
    ...
    for (m = 1; m <= n; m++) {
    	...
    	if (t == 1) {          // testing if the Orbit is equal to 1, 
		                       //       then m is a factor of n
             cout << "The number " << m << " is a factor of " << n << endl;
        }
    ...
    value_if_prime = n*(n-1)/2 + 1;      // formula for the total of all Orbits 
	                                         //          if n is prime
    cout << "If n is Prime, then the total of all Orbits On(m) should be ";
